...
async def test_register_returns_user_without_password(client: AsyncClient) -> None:
    r = await client.post("/auth/register", json={"email": "u@x.co", "password": PWD})
...
async def test_register_duplicate_email_is_409(client: AsyncClient) -> None:
    await client.post("/auth/register", json={"email": "dup@x.co", "password": PWD})
    r = await client.post("/auth/register", json={"email": "dup@x.co", "password": PWD})
    assert r.status_code == 409
    assert r.json()["code"] == "email_already_registered"


async def test_register_weak_password_is_422(client: AsyncClient) -> None:
    r = await client.post("/auth/register", json={"email": "w@x.co", "password": "short"})
    assert r.status_code == 422


async def test_login_wrong_password_is_401(client: AsyncClient) -> None:
    await client.post("/auth/register", json={"email": "l@x.co", "password": PWD})
    r = await client.post("/auth/login", data={"username": "l@x.co", "password": "wrong"})
    assert r.status_code == 401
    assert r.json()["code"] == "invalid_credentials"


async def test_login_unknown_user_is_401(client: AsyncClient) -> None:
    r = await client.post("/auth/login", data={"username": "ghost@x.co", "password": PWD})
    assert r.status_code == 401
...
async def test_me_returns_current_user(member_client: AsyncClient) -> None:
    r = await member_client.get("/auth/me")
...
async def test_admin_route_forbidden_for_member(member_client: AsyncClient) -> None:
    r = await member_client.get("/admin/users")
...
async def test_member_cannot_touch_another_members_task(app: FastAPI) -> None:
    ...
    try:
        pid = (await alice.post("/projects", json={"name": "secret"})).json()["id"]
        tid = (await alice.post("/tasks", json={"title": "privé", "project_id": pid})).json()["id"]

        # Bob : 404 (et NON 403) — on ne révèle pas l'existence de la ressource
        assert (await bob.get(f"/tasks/{tid}")).status_code == 404
        assert (await bob.patch(f"/tasks/{tid}", json={"status": "done"})).status_code == 404
        assert (await bob.delete(f"/tasks/{tid}")).status_code == 404
        assert (await bob.get("/tasks")).json()["total"] == 0
        assert (await bob.get(f"/projects/{pid}")).status_code == 404
...
async def test_admin_sees_every_task(app: FastAPI, admin_client: AsyncClient) -> None:
    ...
    try:
        pid = (await member.post("/projects", json={"name": "P"})).json()["id"]
        (await member.post("/tasks", json={"title": "t", "project_id": pid}))
        # l'admin (fixture) voit la tâche du membre
        page = (await admin_client.get("/tasks")).json()
        assert page["total"] == 1
