...
from taskman.api.idempotency import IdempotencyMiddleware
...
from taskman.api.routes import admin, auth, events, meta, ops, projects, tasks, v2
...
from taskman.realtime import build_event_publisher
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    app.state.event_publisher = build_event_publisher(settings.redis_url)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    # Infrastructure — non versionnée.
    app.include_router(meta.router)
    app.include_router(ops.router)

    # API métier — versionnée. /v1 et /v2 coexistent.
