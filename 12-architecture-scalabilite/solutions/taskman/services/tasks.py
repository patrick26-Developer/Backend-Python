...
from taskman.outbox import OutboxRepository
...
class TaskService:
    # Beaucoup de collaborateurs -> le signe qu'on gagnerait à séparer commandes
    # et requêtes (CQRS léger). Assumé ici pour garder un seul point d'entrée.
    def __init__(
        self,
    ...
    async def complete(self, task_id: int) -> TaskRead:
        ...
        await self._outbox.add(event)  # même transaction que la tâche (outbox pattern)
        await self._uow.commit()  # tâche + événement : tout ou rien
        await self._invalidate_project(task.project_id)
        await self._events.publish(
            event
        )  # temps réel best-effort (SSE) ; l'outbox reste la source sûre
        return task
