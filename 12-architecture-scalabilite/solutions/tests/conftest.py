...
async def app(session_factory: async_sessionmaker[AsyncSession]) -> AsyncIterator[FastAPI]:
    application = create_app(
        Settings(
            ...
            log_json=False,
            log_level="WARNING",
            rate_limit_enabled=False,  # les tests dédiés le réactivent
        )
    )
    # Le `lifespan` ne tourne pas avec ASGITransport : on pose ce dont les
    # dépendances ont besoin dans `app.state` à la main.
    application.state.cache = InMemoryCache()
    application.state.rate_limiter = InMemoryRateLimiter()
    application.state.event_publisher = InMemoryEventPublisher()
...
async def _authenticate(c: AsyncClient, email: str) -> None:
    ...
    resp = await c.post("/v1/auth/login", data={"username": email, "password": PWD})
...
async def admin_client(
    ...
    async with _new_client(app) as c:
        await c.post("/v1/auth/register", json={"email": "admin@test.co", "password": PWD})
        async with session_factory() as s:
            await s.execute(
                update(UserRow).where(UserRow.email == "admin@test.co").values(role=UserRole.admin)
            )
            await s.commit()
        resp = await c.post("/v1/auth/login", data={"username": "admin@test.co", "password": PWD})
        c.headers["Authorization"] = f"Bearer {resp.json()['access_token']}"
        yield c
