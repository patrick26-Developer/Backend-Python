...
async def test_auth_rate_limit_returns_429(limited_client: AsyncClient) -> None:
    ...
    for i in range(5):
        r = await limited_client.post(
            "/v1/auth/login", data={"username": f"x{i}@y.co", "password": PWD}
        )
        codes.append(r.status_code)
    # 3 tentatives autorisées (401), puis 429
    assert codes.count(429) >= 1
    last = await limited_client.post("/v1/auth/login", data={"username": "z@y.co", "password": PWD})
...
async def test_rate_limit_is_per_ip(limited_client: AsyncClient) -> None:
    for _ in range(4):
        await limited_client.post("/v1/auth/login", data={"username": "a@y.co", "password": PWD})
...
async def test_oversized_payload_is_413(client: AsyncClient) -> None:
    ...
    resp = await client.post("/v1/auth/register", json={"email": "a@b.co", "password": huge})
