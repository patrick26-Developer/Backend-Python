...
async def test_project_stats(member_client: AsyncClient) -> None:
    ...
    t1 = (await member_client.post("/tasks", json=task_payload(project_id=pid))).json()["id"]
    await member_client.post("/tasks", json=task_payload(project_id=pid))
    await member_client.post(f"/tasks/{t1}/complete")

    stats = (await member_client.get(f"/projects/{pid}/stats")).json()
...
async def test_stats_of_unknown_project_is_404(member_client: AsyncClient) -> None:
    assert (await member_client.get("/projects/999/stats")).status_code == 404


async def test_stats_cache_invalidated_on_new_task(member_client: AsyncClient) -> None:
    pid = await _project(member_client)
    await member_client.post("/tasks", json=task_payload(project_id=pid))
    assert (await member_client.get(f"/projects/{pid}/stats")).json()["total"] == 1
    await member_client.post("/tasks", json=task_payload(project_id=pid))
    # sans invalidation, on lirait encore "1" depuis le cache
    assert (await member_client.get(f"/projects/{pid}/stats")).json()["total"] == 2
...
async def test_bad_cursor_is_400(member_client: AsyncClient) -> None:
    resp = await member_client.get("/tasks", params={"cursor": "pas-du-base64-valide!!"})
...
async def test_export_ndjson_streamed(member_client: AsyncClient) -> None:
    pid = await _project(member_client)
    for i in range(3):
        await member_client.post("/tasks", json=task_payload(project_id=pid, title=f"export {i}"))

    resp = await member_client.get("/tasks/export")
...
async def test_export_only_own_tasks(app) -> None:  # type: ignore[no-untyped-def]
    ...
    try:
        ...
        await alice.post("/tasks", json=task_payload(project_id=pid))
        assert len([x for x in (await alice.get("/tasks/export")).text.splitlines() if x]) == 1
        assert (await bob.get("/tasks/export")).text.strip() == ""
    finally:
        await alice.aclose()
        await bob.aclose()
