...
def _task_svc(repo: InMemoryTaskRepository, actor: UserRead) -> TaskService:
    return TaskService(repo, NullUnitOfWork(), actor, InMemoryCache())
...
async def test_commit_on_write_not_read() -> None:
    ...
    service = TaskService(InMemoryTaskRepository(), SpyUoW(), _user(), InMemoryCache())
...
async def test_stats_are_cached_then_invalidated() -> None:
    repo = InMemoryTaskRepository()
    cache = InMemoryCache()
    svc = TaskService(repo, NullUnitOfWork(), _user(), cache)
...
async def test_stats_completion_rate() -> None:
    repo = InMemoryTaskRepository()
    svc = TaskService(repo, NullUnitOfWork(), _user(), InMemoryCache())
...
async def test_export_yields_all_owner_tasks() -> None:
    repo = InMemoryTaskRepository()
    alice = TaskService(repo, NullUnitOfWork(), _user(uid=1), InMemoryCache())
    bob = TaskService(repo, NullUnitOfWork(), _user(uid=2), InMemoryCache())
