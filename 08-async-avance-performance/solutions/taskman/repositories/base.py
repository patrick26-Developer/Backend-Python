...
class TaskRepository(Protocol):
    ...
    # Pagination *keyset* : les `limit` tâches suivant `after` = (created_at, id),
    # triées par (created_at, id) décroissants.
    async def list_keyset(
        self, *, owner_id: int | None, limit: int, after: tuple[datetime, int] | None
    ) -> list[TaskRead]: ...

    # Itère sur toutes les tâches (export streamé) — `def`, pas `async def` :
    # une fonction génératrice async a pour type `(...) -> AsyncIterator[...]`.
    def iter_by_owner(self, owner_id: int | None) -> AsyncIterator[TaskRead]: ...

    # (nombre par statut, nombre en retard) — agrégation SQL en 2 requêtes.
