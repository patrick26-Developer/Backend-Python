...
class TaskPage(BaseModel):
    ...
    limit: int
    offset: int
    # Module 08 : jeton opaque pour la page suivante en mode *cursor*.
    # `None` en mode offset, ou quand il n'y a plus de page.
...
class TaskFilters(BaseModel):
    ...
    # Si fourni, pagination *keyset* par (created_at, id) décroissants : `sort` et
    # `offset` sont ignorés. Jeton renvoyé dans `TaskPage.next_cursor`.
    cursor: str | None = Field(default=None, max_length=200)


class TaskStats(BaseModel):
    """Agrégats d'un projet — coûteux à calculer, donc mis en cache (Module 08)."""
